...
def retriever_source(question: str, session: Session, top_k: int = 3) -> str:
    ...
    stmt = (
        select(CsGuideDocument.title, CsGuideDocumentChunk.page, CsGuideDocumentChunk.content, distance.label("distance"))
        .join(CsGuideDocument, CsGuideDocument.id == CsGuideDocumentChunk.cs_guide_document_id)
        .order_by(distance)
        .limit(top_k)
    )
    ...
    # page 는 페이지 개념이 없는 원본에서 NULL 이라 그때는 제목만 남긴다
    return "\n\n".join(
        f"[출처: {r.title[:20]}{f' {r.page}p' if r.page is not None else ''}]\n{r.content}"
        for r in rows
    )