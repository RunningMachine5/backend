...
### Customer chatbot — read the design docs first

The customer-chatbot design lives in `docs/customer-chatbot/` (5 files). Before touching
`app/services/{chatbot,rag}/`, `app/pipelines/customer_chatbot_pipeline.py`, `app/api/chat.py`,
or the chat tables in `app/data/model/agent.py`, open
[docs/customer-chatbot/01-overview.md](docs/customer-chatbot/01-overview.md) and use its index to
find the relevant section. `04-prompts.md` is the single source for every customer-facing message,
chatbot question, and LLM prompt — never invent new wording in code. For a full implementation
task, invoke the `chatbot-feature` skill.
