...
유사 사례 최소 유사도는 `0.55`, `0.60`, `0.65`를 각각 적용한다. 시연용 Seed 평가에서는
`0.55`가 Top-1 정확도 100%를 유지하면서 검색 커버리지 94.44%로 가장 높아, 현재 기본값으로
적용했다. 유사도 가중치 자체는 현재 운영 기준을 유지하고, 이번 평가에서는 후보가 늘어나는
것을 막기 위해 최소 유사도만 비교한다.
