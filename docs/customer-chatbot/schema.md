...
[턴 응답 뒤 백그라운드로](README.md#비동기-실행))
...
"허용된 enum 이외의 값은 생성하지 않습니다" 같은 프롬프트 규칙은 LLM에 대한 요청일 뿐
강제가 아니다. 모델이 이를 어길 가능성은 항상 있으므로 **구조화 출력 스키마와 DB 저장은
파이썬 코드로 강제한다.** 강제가 여기 있으므로 A.3 프롬프트에서는 그 규칙을 뺐다
([출력 원칙](prompts.md#프롬프트에-적용한-출력-원칙)).
