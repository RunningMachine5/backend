...
세 번째 응답에도 [B.3](messages.md#b3-평가-판정별-안내-문구)의 `REFUSAL` 문구
...
`is_adopted`는 [스키마 3.5](schema.md#35-chat_answers--신규)의 정의대로 **그 응답이
추출 입력으로 쓰였는지**를 뜻한다. 따라서 아래 두 경우에만 `true`다.

| 판정 | `is_adopted` | 이유 |
| --- | --- | --- |
| `SUFFICIENT` | `true` | [2.5](#25-정보-응답--rag-대응-가이드-4-1) · [2.6](#26-사기-정황-추출과-채점-4-2)의 추출 입력이 된다 |
| 재시도 초과(3번째 `TOO_VAGUE` / `NON_ANSWER`) | `true` | 위의 조건 1이 채택을 명시한다. 추출은 돌리지 않는다 |
| `REFUSAL` | `false` | 답변 내용이 없어 추출에 쓰지 않는다 |
| `WANT_END` | `false` | 질문에 답한 것이 아니라 상담을 끝내겠다는 의사다 |
| 재질문 대상 시도(1·2번째) | `false` | 뒤이은 시도가 대신한다 |

부분 유니크 인덱스는 질문당 채택을 **최대 하나**로만 강제하므로, `REFUSAL`처럼 채택
없이 끝나는 질문 단계가 있어도 위반이 아니다.
