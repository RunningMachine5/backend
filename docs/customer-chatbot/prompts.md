...
사용처: [2.5 RAG 단계: 액션별 Retrieve/Augment, Generate는 1회 통합](README.md#rag-단계-액션별-retrieveaugment-generate는-1회-통합)

액션 개수와 무관하게 **LLM 호출은 1회**다. 근거 청크를 찾은 액션(`grounded`)만 이
프롬프트에 들어간다. 검색 0건 액션은 프롬프트에 넣지 않고 애플리케이션 코드가
[B.5](messages.md#b5-안내를-만들지-못한-액션-안내) 고정 문구로 채운다
([2.5 검색 결과 0건 처리](README.md#검색-결과-0건-처리)).

**소제목과 목록 조립은 LLM이 하지 않는다.** LLM은 액션별 안내 본문만 만들고,
소제목(`CUSTOMER_ACTION_DESCRIPTIONS`)과 순서는 코드가 붙인다. 이렇게 해야 고객이 말한
행동이 근거 유무와 무관하게 전부, 말한 순서대로 응답에 나타난다.

`{action_context_block}`은 코드가 조립하며 액션 하나가 아래 형식의 한 블록이 된다.
`행동 설명`은 `CUSTOMER_ACTION_DESCRIPTIONS`, `고객 발언`은 추출 결과의 `evidence`,
`근거`는 액션별 검색 결과 청크(액션당 최대 3개)다.

```text
[1] phishing_link_opened
행동 설명: 상대방이 보낸 의심스러운 링크를 열거나 누름
...
`guidance`가 빈 문자열이거나 응답에서 빠진 행동은 근거가 있어도 안내를 만들지 못한 것이므로
[B.5](messages.md#b5-안내를-만들지-못한-액션-안내) 문구로 대체한다. `type`의 허용값은 A.2와
같은 방식으로 파이썬 코드가 강제한다([스키마 3.8](schema.md#38-appdomain-enum-코드-상수화)).
