...
**스키마가 강제하는 것은 프롬프트에 쓰지 않는다.** 네 호출 모두
[build_structured_llm](../../app/services/chatbot/llm.py)의 `strict=True` `json_schema`를 쓰므로
출력 형식·enum 화이트리스트는 제약 디코딩으로 강제된다. 프롬프트가 같은 것을 다시 요구하면
모델이 검사할 항목만 늘어 추론 토큰이 커진다([A.2 규칙을 줄인 이유](#규칙을-줄인-이유)의 측정 근거).
그래서 아래를 네 절에서 뺐다.

| 뺀 것 | 어디에 있었나 | 대신 보장하는 것 |
| --- | --- | --- |
| `출력 형식: { ... }` JSON 예시 블록 | A.2 · A.3 · A.4 | 각 Pydantic 응답 스키마가 그대로 전송된다. 필드의 의미는 규칙 목록이 설명한다 |
...
- A.4의 `근거에 없는 절차를 만들어내지 않는다`가 정상 작동해 질의 2의 guidance가 비었고,
  [B.5](messages.md) 폴백이 응답에 붙었다.
