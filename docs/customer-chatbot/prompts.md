...
A.2는 챗봇의 네 LLM 호출 중 유일한 생성 작업이라 언제나 가장 느리다. 나머지는 분류(A.1)와
enum 선택(A.3)이라 출력이 짧게 끝나는 반면, A.2는 최대 5개 객체 × 자유 텍스트 3필드를
지어낸다. 규칙이 많을수록 열린 출력 공간에서 검사할 것이 늘어 추론 토큰까지 함께 커진다
(모델별 측정값은 [PRD 2.4](README.md#모델reasoning-effort와-타임아웃-예산)).

**형식 제약은 프롬프트에서 뺐다. 구조화 출력 스키마가 이미 서버에서 강제하기 때문이다.**
`strict=True`의 `json_schema`는 `maxItems`/`maxLength`를 제약 디코딩으로 강제한다(8개·30자를
요구해도 스키마대로 2개·10자로 잘리는 것을 확인했다). 따라서 아래는 프롬프트에 적을 필요가
없다.

| 뺀 규칙 | 대신 보장하는 것 |
| --- | --- |
| 최대 5개만 반환 | `GuideSearchQueryExtractionResult.guide_search_queries`의 `max_length=5`. 프롬프트에는 **개수를 채우지 말라**는 지시만 남긴다 |
