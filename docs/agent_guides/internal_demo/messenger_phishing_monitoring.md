...
원격제어가 확인되면 고객이 화면 공유나 원격지원 앱을 실행했는지 확인한다. 인증정보 변경과
원격제어가 함께 적중하면 거래 전후 인증수단 변경 시각을 함께 검토한다. 오픈뱅킹 사용과
단시간 반복송금이 적중하면 수취계좌 수와 반복 간격을 확인한다. 고령자 모바일 환경에서 본인
계좌가 아닌 곳으로 송금한 정황만 있는 경우에는 가족·지인 사칭 여부를 추가 확인하되, 해당
조건만으로 메신저피싱을 확정하지 않는다.

수취계좌 송금과 원격제어가 함께 확인되면 고객 거래 진위, 원격제어 앱 사용 여부, 추가 송금
진행 여부를 우선순위로 제시한다.
